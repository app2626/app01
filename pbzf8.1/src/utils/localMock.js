...
// อีเมลต้อง normalize (ตัดช่องว่าง/ตัวพิมพ์เล็ก) ก่อนบันทึก/เปรียบเทียบเสมอ — mirror ของ normalizeEmail_ ใน Code.js
function normalizeEmailLocal_(email) {
  return String(email || "").trim().toLowerCase();
}

export function registerMemberLocal({ name, email, password, requestAdmin }) {
  email = normalizeEmailLocal_(email);
...
export function placeOrderLocal(data) {
  ...
  // ตรวจรายการสินค้าก่อน: qty ต้องเป็นจำนวนเต็มบวก และราคาใช้ราคาจริงจาก products array เท่านั้น (mirror ของ Code.js)
  ...
  for (const item of data.items) {
    ...
    if (!Number.isInteger(qty) || qty < 1 || qty > MAX_ITEM_QTY_LOCAL_) {
      return { success: false, message: `จำนวนสินค้า "${item.name || item.sku || ""}" ไม่ถูกต้อง (ต้องเป็น 1-${MAX_ITEM_QTY_LOCAL_} ชิ้น)` };
    }
  ...
  const shortages = checkStockAvailableLocal_(trustedItems.concat(giftStockItemsLocal_(trustedItems, data.channel)));
  if (shortages.length) {
    const s = shortages[0];
  ...
  // ตรวจคูปองอย่างเดียวก่อน — ตัดสิทธิ์ (usedCount += 1) หลังเช็คแต้มสะสมผ่านแล้วเท่านั้น
  // กันคูปองถูกนับใช้ไปแล้วทั้งที่ออเดอร์ล้มเหลวเพราะแต้มไม่พอ (mirror ของ Code.js)
  ...
  if (data.couponCode) {
    ...
    const customerEmail = memberEmail || normalizeEmailLocal_(data.email);
  ...
  if (appliedCoupon) appliedCoupon.usedCount += 1;
  ...
  giftStockItemsLocal_(trustedItems, data.channel).forEach(g => {
  ...
  orders.push({
    ...
    orderId,
    items: trustedItems,
    email: normalizeEmailLocal_(data.email),
...
// จำกัดจำนวนครั้งเดารหัสยืนยันผิด — mirror ของ resetPassword ฝั่ง Code.js
const resetFailuresLocal_ = new Map(); // email -> { count, resetAt }

export function resetPasswordLocal(email, code, newPassword) {
  email = normalizeEmailLocal_(email);
  const failure = resetFailuresLocal_.get(email);
  if (failure && failure.count >= LOGIN_FAIL_LIMIT_LOCAL_ && Date.now() < failure.resetAt) {
  ...
  if (!member.resetCode || member.resetCode !== String(code)) {
    const current = resetFailuresLocal_.get(email);
    const count = (current && Date.now() < current.resetAt ? current.count : 0) + 1;
    resetFailuresLocal_.set(email, { count, resetAt: Date.now() + LOGIN_LOCKOUT_MS_LOCAL_ });
  ...
  resetFailuresLocal_.delete(email);
...
export function saveProductLocal(product, token) {
  ...
  // mirror ของ saveProduct ฝั่ง Code.js — ห้ามบันทึกสินค้าที่ไม่มี variant / SKU ว่าง / SKU ซ้ำ
  ...
  for (const p of products) {
    ...
    for (const v of (p.variants || [])) {
      const sku = String(v.sku || "").trim();
      if (sku && seenSkus[sku]) return { success: false, message: `SKU "${sku}" ถูกใช้กับสินค้าอื่นอยู่แล้ว` };
    }
  }
...
// mirror ของ setDepositStatus ฝั่ง Code.js — แอดมินอัปเดตสถานะมัดจำหลังจอง
export function setDepositStatusLocal(orderId, status, token) {
  if (!requireAdminLocal_(token)) return { success: false, message: "ไม่มีสิทธิ์เข้าถึง" };
