...
function CategoryTiles({ activeSlug, onSelectCategory }) {
  return (
      ...
      {CATEGORIES.map(cat => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center gap-1.5 flex-shrink-0 w-20 py-2 rounded-lg border transition-colors ${
              active ? "border-[#FFD700] bg-[#FFFBE6]" : "border-gray-200 bg-white hover:border-gray-300"
            }`}
          >
            <Icon className={`w-5 h-5 ${active ? "text-[#B8860B]" : "text-gray-500"}`} />
            <span className={`text-[10px] font-medium text-center leading-tight ${active ? "text-[#B8860B]" : "text-gray-600"}`}>
              {cat.label}
            </span>
          </button>
        );
...
export default function CategoryPage({ categorySlug, allProducts, filters, onFiltersChange, onProductClick, onGoHome, onSelectCategory, compareList, onToggleCompare }) {
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start pb-10 pt-4 px-4 gap-6">
      ...
      <div className="flex-1 w-full">
        ...
        {onSelectCategory && <CategoryTiles activeSlug={categorySlug} onSelectCategory={onSelectCategory} />}
