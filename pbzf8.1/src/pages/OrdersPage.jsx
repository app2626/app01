...
export default function OrdersPage({ member, onGoHome }) {
  ...
  if (!member) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center text-gray-500">
        กรุณาเข้าสู่ระบบเพื่อดูรายการจองของคุณ
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6">
      <h1 className="text-xl font-bold text-gray-900 mb-6">รายการจองของฉัน</h1>

      {loading ? (
        <div className="text-center text-gray-500 py-12">กำลังโหลด...</div>
      ) : loadError ? (
        <div className="text-center text-red-500 py-12">ไม่สามารถโหลดรายการจองได้ กรุณาลองใหม่อีกครั้ง</div>
      ) : orders.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-200 p-12 flex flex-col items-center text-center">
          <Package className="w-12 h-12 text-gray-300 mb-4" />
          <h3 className="text-lg font-bold text-gray-900 mb-2">ยังไม่มีรายการจอง</h3>
          <button
            onClick={onGoHome}
              ...
              <div className="mt-2 pt-2 border-t border-gray-100 space-y-1">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-900">
                    {formatTHB(order.depositAmount)}{" "}
                    {order.depositStatus === "paid" ? (
                      <span className="text-green-600 text-xs">(ชำระแล้ว)</span>
                    ) : (
                      <span className="text-amber-600 text-xs">(ยังไม่ชำระ)</span>
                    )}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-900">
                    {formatTHB(order.remainingAmount)}{" "}
                    {order.remainingPaid ? (
                      <span className="text-green-600 text-xs">(ชำระแล้วเมื่อ {new Date(order.remainingPaidAt).toLocaleDateString("th-TH")})</span>
                    ) : (
                      <span className="text-amber-600 text-xs">(รอชำระตอนรับสินค้า)</span>
                    )}
                  </span>
                </div>
              </div>
