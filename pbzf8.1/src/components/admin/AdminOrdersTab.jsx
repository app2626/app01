...
import { getAdminOrdersLocal, updateOrderStatusLocal, updateOrderTrackingLocal, markRemainingPaidLocal, setDepositStatusLocal } from "../../utils/localMock";
...
export default function AdminOrdersTab({ member }) {
  ...
  const handleStatusChange = async (orderId, status) => {
    // อัปเดตหน้าจอทันที (optimistic) แต่ถ้าเซิร์ฟเวอร์บันทึกไม่สำเร็จต้อง revert กลับค่าเดิม
    // ไม่งั้นหน้าจอจะโชว์สถานะที่ไม่ตรงกับข้อมูลจริงในระบบ
    ...
    try {
      ...
      setOrders(prev => prev.map(o => o.orderId === orderId ? { ...o, status: prevStatus } : o));
      alert("เชื่อมต่อระบบไม่สำเร็จ กรุณาลองใหม่");
    }
  };

  const handleMarkDepositPaid = async (orderId) => {
    try {
      const res = await callGas("setDepositStatus", [orderId, "paid", member.token], () => setDepositStatusLocal(orderId, "paid", member.token));
      if (res.success) {
        setOrders(prev => prev.map(o => o.orderId === orderId ? { ...o, depositStatus: "paid" } : o));
      } else {
              ...
              {orders.map(o => (
                <tr key={o.orderId} className="border-b border-gray-50 last:border-0">
                  ...
                  <td className="p-3">
                    ...
                    {o.depositStatus !== "paid" && o.status !== "ยกเลิก" && (
                      <button
                        onClick={() => handleMarkDepositPaid(o.orderId)}
                        className="text-xs text-[#B8860B] hover:underline mt-0.5"
                      >
                        ทำเครื่องหมายว่ามัดจำแล้ว
                      </button>
                    )}
