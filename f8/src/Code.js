...
function setupDatabase(userToken) {
  ...
  const tables = {
    ...
    // Title/Button Text (2026-09-18) — ใช้เฉพาะ popupbanner สำหรับหัวข้อ+ปุ่ม CTA (hero/grid banner เว้นว่างไว้ ไม่กระทบ)
    "UI_Banners": ["Banner ID", "Type", "URL", "Status", "Target Link", "Details", "Title", "Button Text"],
    "Target": ["Area", "เป้า"],
...
function apiHandler(action, payload, userToken) {
  try {
    ...
    switch(action) {
      ...
      case 'SAVE_PRODUCT_GROUP': return saveProductGroup(payload, secureUser, ss);
      case 'UPDATE_PRODUCT_GROUP': return updateProductGroup(payload, secureUser, ss);
      ...
      case 'GET_CUSTOMER_BY_PHONE': return getCustomerByPhone(payload, ss); // ทุก role ที่ login แล้ว — คืนเฉพาะฟิลด์เติมฟอร์ม
...
// เติมคอลัมน์ Title/Button Text ให้ชีต UI_Banners ที่สร้างไว้ก่อนฟีเจอร์นี้ (2026-09-18) — เขียนแค่ header แถวแรก
// ข้อมูลแถวเดิมไม่ต้องแตะ (data[i][6]/data[i][7] อ่านได้ปกติเป็น undefined เมื่อยังไม่มีคอลัมน์ ไม่พังอยู่แล้ว)
function ensureBannerColumns_(sheet) {
  if (sheet.getLastColumn() >= 8) return;
  const headers = sheet.getRange(1, 1, 1, Math.max(sheet.getLastColumn(), 6)).getValues()[0];
  if (headers.length < 7 || !headers[6]) sheet.getRange(1, 7).setValue("Title");
  if (headers.length < 8 || !headers[7]) sheet.getRange(1, 8).setValue("Button Text");
}
...
function getAllInitialData(ss) {
  ...
  try {
    ...
    if(bannersSheet) {
      ensureBannerColumns_(bannersSheet);
      ...
      for(let i=1; i<data.length; i++) {
        ...
        const bannerTitle = data[i][6] ? data[i][6].toString().trim() : "";
        const buttonText = data[i][7] ? data[i][7].toString().trim() : "";
        if (status !== 'Deleted' && url && url.toString().trim() !== "") {
          ...
          else if (type === 'popupbanner') popupBanners.push({url: url.toString().trim(), link: targetLink, details: details, title: bannerTitle, buttonText: buttonText});
        }
      }
    } else {
    ...
    payload.cart.forEach(item => {
      ...
      const processGift = (giftObj) => {
        if(giftObj && giftObj.name) {
          ...
          if (!Number.isInteger(gQty) || gQty < 1) throw new Error("จำนวนของแถมไม่ถูกต้อง: " + giftObj.name);
          let giftSku = "GIFT";
...
function getCustomerByPhone(payload, ss) {
  ...
  try {
    ...
    matchingRows.forEach(item => {
      ...
      if (r === rowIdx) {
        ...
        let unitPriceRaw = dataObj["Unit Price"] !== undefined ? dataObj["Unit Price"] : rData[oHeaders.indexOf("Unit Price")];
        let unitPrice = parseFloat((unitPriceRaw || 0).toString().replace(/,/g, ''));
...
function saveRecord(tableName, dataObj, idField, secureUser, ss) {
  ...
  try {
    ...
    if(tableName === "Members") { if (!dataObj.Password || dataObj.Password.trim() === "") { if (rowIndex > -1) { dataObj.Password = data[rowIndex - 1][headers.indexOf("Password")]; } else { dataObj.Password = hashPassword("1234"); } } else { if (dataObj.Password.length < 50) dataObj.Password = hashPassword(dataObj.Password); } }
...
// เพิ่มสินค้าใหม่แบบหลาย SKU ในครั้งเดียว (พอร์ตแนวคิดจากโปรเจกต์ pbzf8's saveProduct) — กรอกข้อมูลร่วมของสินค้า
// (Product Name/Model/Product Group/Image URL/ฯลฯ) ครั้งเดียว แล้วเพิ่มหลายแถว SKU (Capacity/Color/SKU/Price/Stock)
// ในคำขอเดียว แทนการเข้าฟอร์ม "เพิ่มข้อมูล" ทีละแถวซ้ำๆ — Model เป็น key จัดกลุ่ม SKU ของสินค้าเดียวกัน
// (แยกจาก Product Group ซึ่งเป็น key ระดับซีรีส์สำหรับผูกโปรฯ/ของแถมข้ามรุ่นใน GiftMappings)
// รองรับ 2 โหมดจาก Model ที่ส่งมา: (1) Model ใหม่ = สร้างสินค้าใหม่ทั้งหมด ใช้ shared ที่ส่งมา
// (2) Model ซ้ำกับสินค้าเดิม = โหมด "เพิ่ม SKU เข้าสินค้าเดิม" — ไม่แตะ/ไม่ลบแถวเดิมเลย เพิ่มแถวใหม่ต่อท้ายเท่านั้น
// และใช้ค่าฟิลด์ร่วมจาก "แถวแรกที่เจอของสินค้านั้น" เสมอ (ไม่ใช้ shared ที่ client ส่งมา) กันข้อมูลไม่ตรงกันระหว่าง SKU เดิม/ใหม่ของสินค้าเดียวกัน

// Product Name ไม่รับกรอกจาก client อีกต่อไป — สร้างอัตโนมัติจาก Model + Capacity + Color ของแต่ละแถว (ตัดส่วนที่ว่างทิ้ง)
// ใช้ร่วมกันทั้ง saveProductGroup (สร้างใหม่/append) และ updateProductGroup (แก้ไข) เพื่อให้ชื่อสินค้าตรงกันเสมอไม่ว่าจะเข้าทางไหน
function buildAutoProductName_(model, capacity, color) {
  return [model, capacity, color].map(s => (s || '').toString().trim()).filter(Boolean).join(' ');
}

function saveProductGroup(payload, secureUser, ss) {
  const lock = LockService.getScriptLock();
  lock.waitLock(10000);
  try {
    if (secureUser.Role !== 'Admin') throw new Error("Permission Denied: Admin only");
    ...
    const model = (shared['Model'] || '').toString().trim();
    if (!model) throw new Error("กรุณาระบุ Model");

    const variantsIn = Array.isArray(payload.variants) ? payload.variants : [];
    if (variantsIn.length === 0) throw new Error("กรุณาเพิ่มอย่างน้อย 1 SKU");

    const sheet = ss.getSheetByName("Products");
    if (!sheet) throw new Error("ไม่พบชีต Products กรุณากด Auto Setup ก่อน");
    const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0].map(h => h.toString().trim());
    const modelIdx = headers.indexOf('Model');
    const skuIdx = headers.indexOf('SKU');
    if (modelIdx === -1 || skuIdx === -1) throw new Error("โครงสร้างชีต Products ไม่ถูกต้อง (ไม่พบคอลัมน์ Model/SKU)");

    const data = sheet.getDataRange().getValues();

    // หา Model เดิม (ถ้ามี) — แถวแรกที่เจอใช้เป็นต้นแบบฟิลด์ร่วมตอน append
    let firstExistingRow = null;
    for (let i = 1; i < data.length; i++) {
      if ((data[i][modelIdx] || '').toString().trim() === model) { firstExistingRow = data[i]; break; }
    }
    const isAppend = !!firstExistingRow;

    let sharedForRows;
    if (isAppend) {
      sharedForRows = {};
      headers.forEach((h, i) => { sharedForRows[h] = firstExistingRow[i]; });
    } else {
    ...
    const existingSkus = {};
    for (let i = 1; i < data.length; i++) {
      const s = (data[i][skuIdx] || '').toString().trim();
      if (s) existingSkus[s] = true;
    }

    const seenSku = {};
    const newRows = variantsIn.map(v => {
      const sku = (v.sku || '').toString().trim();
      if (!sku) throw new Error("กรุณาระบุ SKU ให้ครบทุกแถว");
      if (seenSku[sku]) throw new Error("SKU ซ้ำกันในฟอร์ม: " + sku);
    ...
    CacheService.getScriptCache().remove("TABLE_Products");
    logAudit(secureUser.Username, isAppend ? "INSERT_PRODUCT_SKUS" : "INSERT_PRODUCT_GROUP",
      (isAppend ? "Added " + newRows.length + " SKU(s) to existing product: " : "Created product: ") + model + " (" + newRows.length + " SKUs)");

    return { status: 'success', model: model, skuCount: newRows.length, appended: isAppend };
  } catch(e) {
    return { status: 'error', message: e.toString() };
  } finally {
    lock.releaseLock();
...
// แก้ไขสินค้าเดิมทั้งกลุ่ม (ทุก SKU ของ Model เดียวกัน) ในคำขอเดียว — คู่กับ saveProductGroup ด้านบน
// ลบแถวเดิมทั้งหมดของ originalModel แล้วเขียนชุด SKU ใหม่ทั้งหมดแทน (เพิ่ม/ลบ/แก้ SKU ได้อิสระในหน้าเดียว เหมือน pbzf8's saveProduct)
// ห้ามเปลี่ยนชื่อ Model ผ่านฟังก์ชันนี้ (ฟอร์มฝั่ง client ล็อกฟิลด์ Model ไว้แล้ว แต่ตรวจซ้ำฝั่ง server) — กัน SKU เดิมหลุดกลุ่ม/ชนกับ Model อื่นโดยไม่ตั้งใจ
function updateProductGroup(payload, secureUser, ss) {
  const lock = LockService.getScriptLock();
  lock.waitLock(10000);
  try {
    if (secureUser.Role !== 'Admin') throw new Error("Permission Denied: Admin only");

    const originalModel = (payload.originalModel || '').toString().trim();
    if (!originalModel) throw new Error("ไม่พบ Model ที่จะแก้ไข");

    const shared = payload.shared || {};
    const submittedModel = (shared['Model'] || '').toString().trim();
    if (submittedModel && submittedModel !== originalModel) throw new Error("ไม่สามารถเปลี่ยนชื่อ Model ผ่านฟอร์มแก้ไขนี้ได้");
    ...
    const data = sheet.getDataRange().getValues();
    const hasExisting = data.some((row, i) => i > 0 && (row[modelIdx] || '').toString().trim() === originalModel);
    if (!hasExisting) throw new Error("ไม่พบสินค้า Model \"" + originalModel + "\" ในระบบ (อาจถูกลบไปแล้ว กรุณารีเฟรชหน้า)");

    // SKU ต้องไม่ซ้ำกับสินค้า "อื่น" ที่ไม่ใช่กลุ่มที่กำลังแก้ไข — แถวเดิมของกลุ่มนี้กำลังจะถูกลบแล้วเขียนใหม่อยู่แล้ว จึงชนกันเองไม่ได้ (อนุญาตให้คง SKU เดิมไว้ได้ตามปกติ)
    const otherSkus = {};
    for (let i = 1; i < data.length; i++) {
      if ((data[i][modelIdx] || '').toString().trim() === originalModel) continue;
      const s = (data[i][skuIdx] || '').toString().trim();
      if (s) otherSkus[s] = true;
    }
    ...
    // ลบแถวเดิมทั้งหมดของ Model นี้ (ไล่จากล่างขึ้นบนกันเลขแถวเลื่อน) แล้วเขียนแถวใหม่ทั้งหมดแทน
    for (let i = data.length - 1; i >= 1; i--) {
      if ((data[i][modelIdx] || '').toString().trim() === originalModel) sheet.deleteRow(i + 1);
    }
    sheet.getRange(sheet.getLastRow() + 1, 1, newRows.length, headers.length).setValues(newRows);
    CacheService.getScriptCache().remove("TABLE_Products");
    logAudit(secureUser.Username, "UPDATE_PRODUCT_GROUP", "Updated product: " + originalModel + " (" + newRows.length + " SKUs)");

    return { status: 'success', model: originalModel, skuCount: newRows.length };
  } catch(e) {
    return { status: 'error', message: e.toString() };
  } finally {
    lock.releaseLock();
...
function getSettingsList(ss) {
  ...
  for(let i=1; i<data.length; i++) {
    ...
    if (status !== 'Deleted' && url && url.toString().trim() !== "") {
      ...
      else if (type === 'popupbanner') popupBanners.push({ rowIndex: rIdx, id: data[i][0] ? data[i][0].toString().trim() : '', url: url.toString().trim(), targetLink: targetLink, details: details, title: bannerTitle, buttonText: buttonText });
    }
  }
...
function saveSettingsItem(payload, secureUser, ss) {
  ...
  try {
    ...
    if (type === 'bookingsettings') {
      ...
      for (let key in settings) {
        ...
        if (!found) {
          ...
          const remarkMap = { ReserveStart: 'วันเริ่มจองสินค้า', ReserveEnd: 'วันสิ้นสุดจองสินค้า', DriveFolderId: 'โฟลเดอร์ Drive เก็บรูปอัปโหลด', InvoiceCompanyName: 'ชื่อบริษัท (ใบเสนอราคา/ใบแจ้งหนี้)', InvoiceCompanyAddress: 'ที่อยู่บริษัท (ใบเสนอราคา/ใบแจ้งหนี้)', InvoiceCompanyEmail: 'อีเมลติดต่อ (ใบเสนอราคา/ใบแจ้งหนี้)', InvoiceLogoUrl: 'โลโก้บริษัท (ใบเสนอราคา/ใบแจ้งหนี้)', ApproverSignatureUrl: 'รูปลายเซ็นผู้อนุมัติ (ประทับเมื่ออนุมัติใบเสนอราคา/ใบแจ้งหนี้)', ApproverName: 'ชื่อผู้อนุมัติ (ประทับเมื่ออนุมัติใบเสนอราคา/ใบแจ้งหนี้)' };
    ...
    const bannerTitle = payload.title ? payload.title.toString().trim() : "";
    const buttonText = payload.buttonText ? payload.buttonText.toString().trim() : "";
    ...
    if (rowIndex) {
      ...
      bannersSheet.getRange(rowIndex, 7).setValue(bannerTitle);
      bannersSheet.getRange(rowIndex, 8).setValue(buttonText);
      logAudit(secureUser.Username, "SETTING_UPDATE", "Updated Setting " + type + " at row " + rowIndex + ": " + url);
    } else {
      bannersSheet.appendRow([generateId('BAN'), type, url, 'Active', targetLink, details, bannerTitle, buttonText]);
