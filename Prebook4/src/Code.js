...
function setupDatabase() {
  ...
  for (let sheetName in tables) {
    ...
    if (!sheet) {
      ...
      sheet.getRange(1, 1, 1, tables[sheetName].length).setFontWeight("bold").setBackground("#FF9800").setFontColor("white");
      sheet.setFrozenRows(1);
      isNewSheet = true;
    }
    
    if (isNewSheet && sheetName === "Members") {
    ...
    if (isNewSheet && sheetName === "Branches") {
...
function updateFullOrder(dataObj, secureUser, ss) {
  ...
  try {
    ...
    let oldSku = oldDataRow[oHeaders.indexOf("SKU")]; 
    let oldQty = parseInt(oldDataRow[oHeaders.indexOf("Qty")] || 0); 
    let oldStatus = oldDataRow[oHeaders.indexOf("Order Status")]; 
    let branch = oldDataRow[oHeaders.indexOf("Branch")]; 

    let newSku = dataObj["SKU"] || oldSku; 
    let newQty = parseInt(dataObj["Qty"] || oldQty); 
    let newStatus = dataObj["Order Status"] || oldStatus;

    const prodData = prodSheet.getDataRange().getValues(); const pSkuIdx = prodData[0].indexOf("SKU"); const pStockIdx = prodData[0].indexOf("Stock");
...
function deleteRecord(tableName, idField, idValue, secureUser, ss) {
  const sheet = ss.getSheetByName(tableName); const data = sheet.getDataRange().getDisplayValues(); const idIndex = data[0].indexOf(idField);
