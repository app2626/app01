...
function loginMember(data) {
  const cache = CacheService.getScriptCache();
  const failKey = "loginfail_" + data.email;
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (!passwordOk) {
      recordLoginFailure_(data.email);
...
function placeOrder(data) {
  ...
  try {
    ...
    const giftStockItems = resolveGiftStockItems_(data.items, data.channel);
    ...
    // ไม่เชื่อราคา/ยอดรวมที่ client ส่งมาเลย คำนวณยอดจริงจากราคาที่บันทึกในชีต Products เท่านั้น
    // กันกรณี client ถูกแก้ให้ส่งราคาปลอม (ต่ำกว่าจริง) หรือส่วนลดเกินจริงมา
    ...
    if (data.couponCode) {
      const couponCheck = validateAndRedeemCoupon_(data.couponCode, memberEmail || data.email);
    ...
    orderFields = {
      orderId, customerName: data.customerName, phone: data.phone, email: data.email || "",
      nationalId: data.nationalId || "", registrationCode: data.registrationCode || "",
      customerInterests: (data.customerInterests || []).join(","),
      items: JSON.stringify(data.items), subtotal: serverSubtotal,
      ...
      reservationType: data.reservationType === "F" ? "F" : "T", staffName: data.staffName || "", staffPhone: data.staffPhone || "",
    ...
    decrementGiftStock_(data.items, data.channel);
  } finally {
    lock.releaseLock();
  }

  logOrderItems_(orderId, data.items, orderFields.createdAt);
  notifyNewOrder_(orderId, data, orderFields.depositAmount, orderFields.remainingAmount);
...
function notifyNewOrder_(orderId, data, depositAmount, remainingAmount) {
  try {
    ...
    const body =
      ...
      `ลูกค้า: ${data.customerName} (${data.phone}, ${data.email})\n` +
      `ประเภทสิทธิ์การจอง: ${data.reservationType === "F" ? "F (สวมสิทธิ์แคมเปญ)" : "T (มัดจำจองจริง)"}\n` +
      `พนักงานรับจอง: ${data.staffName} (${data.staffPhone})\n` +
      `เลขที่ใบเสร็จรับเงิน: ${data.receiptNumber}\n\n` +
      `รายการสินค้า:\n${itemLines}\n\n` +
      `ยอดรวม: ฿${Number(data.total).toLocaleString("th-TH")}\n` +
      `มัดจำที่เก็บ: ฿${Number(depositAmount).toLocaleString("th-TH")} (${data.depositStatus === "paid" ? "มัดจำแล้ว" : "ยังไม่มัดจำ"})\n` +
      `ยอดคงเหลือเก็บตอนรับสินค้า: ฿${Number(remainingAmount).toLocaleString("th-TH")}`;
  ...
  for (let i = data.length - 1; i >= 1; i--) {
    if (data[i][phoneCol] === phone) {
      return {
        ...
        nationalId: nationalIdCol !== -1 ? (data[i][nationalIdCol] || "") : "",
        registrationCode: registrationCodeCol !== -1 ? (data[i][registrationCodeCol] || "") : ""
      };
    }
  }
  return { found: false };
...
function getProductsData() {
  ...
  const products = Object.keys(rowsById).map(id => {
    ...
    return {
      ...
      specList: first.specList ? JSON.parse(first.specList) : [],
      colors: colors,
      colorHex: colorHex,
      variants: variants,
