...
function doGet(e) {
  ...
  return template.evaluate()
    .setTitle('ACT SAMSUNG System V6.8')
...
function getStockByBranch(locCode) {
  ...
  const actSheet = findSheet(ss, "ระหว่างดำเนินการแอค");
...
function checkIsTimeValid(startStr, endStr) {
  ...
  try {
    ...
    let st = startStr.toString().split(":");
    let startMinutes = parseInt(st[0])*60 + parseInt(st[1]||0);
    let en = endStr.toString().split(":");
    let endMinutes = parseInt(en[0])*60 + parseInt(en[1]||0);
...
function getRecords() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = findSheet(ss, "ระหว่างดำเนินการแอค");
...
function saveBatchRecords(payload) {
  ...
  const targetSheet = findSheet(ss, "ระหว่างดำเนินการแอค");
  if(!targetSheet) return { success: false, message: "ไม่พบฐานข้อมูลชีต 'ระหว่างดำเนินการแอค'" };
  let usedImeis = new Set();
  ...
  payload.items.forEach((item, index) => {
    ...
    } else if(!validStockKeys.has(checkKey)) {
      errors.push(`รายการที่ ${index + 1} (IMEI: ${item.imei}) ไม่มีในสต๊อกของสาขานี้`);
    } else {
      rowsToInsert.push([
